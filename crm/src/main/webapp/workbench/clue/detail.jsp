...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#activitySearchBody").on("click", $("input[name=xz]"), function () {
			$("#activitySelectAll").prop("checked", $("input[name=xz]").length == $("input[name=xz]:checked").length)
		});
		...
		$.ajax({
			...
			success : function (data) {
				...
				$.each(data, function (i, n) {
					...
					html += ' 	<td>' + n.endDate + '</td> ';
					html += ' 	<td>' + n.owner + '</td> ';
					html += ' 	<td><a href="javascript:void(0);" onclick = "if(confirm(\'' + '确认解除关联？' + '\'))unbund(\'' + n.id + '\')" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td> ';
					html += ' </tr> ';
